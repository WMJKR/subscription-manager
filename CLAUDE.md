...
## 2026-07-22 (모바일 QR 시연 중 발견한 버그 2건 수정)

QR로 실제 모바일 환경에서 시연하다 발견된 버그 2건을 수정했다.

### 버그 1 — 토글 스위치 노브가 트랙 밖으로 벗어남
- `/register`의 "무료체험으로 시작하나요?"/"다른 사람과 나눠 쓰나요?" 토글 스위치(`role="switch"`) 2개에서, 꺼짐 상태일 때 노브(동그란 손잡이)가 이미 트랙 우측에 붙어있고, 켜짐 상태로 바꾸면 트랙 바깥으로 튀어나가는 문제
- **원인**: 노브 `<span>`에 `absolute`만 걸고 `left`값을 명시하지 않은 채 `translate-x-0.5`(꺼짐)/`translate-x-5`(켜짐)로만 위치를 잡고 있었음. `left`/`right`가 모두 `auto`일 때 절대위치 요소의 기준 위치("static position")를 브라우저가 어떻게 계산하는지가 기대(좌측 0)와 실제 렌더링(우측 정렬)이 달라서, 두 translate 값 모두 거기서 추가로 이동해 켜짐 상태에서 실제로 트랙(44px) 밖으로 20px 가까이 벗어남 — Playwright로 두 토글의 노브/트랙 `getBoundingClientRect()`를 직접 측정해 확인(꺼짐 시 노브 x=326, 트랙 우측=346로 이미 딱 붙어있었고, 켜짐 시 노브 우측=364로 트랙보다 18px 초과)
- **수정**: 노브에 `left-0.5`를 명시해 기준 위치를 좌측 2px로 고정하고, translate 값을 꺼짐 `translate-x-0`/켜짐 `translate-x-5`로 변경 — 브라우저의 암묵적 static position 계산에 의존하지 않도록 함. 두 토글 모두 동일하게 수정
- 수정 후 재측정: 꺼짐 시 노브 x=304(트랙 좌측에서 2px), 켜짐 시 노브 x=324~344(트랙 우측에서 2px) — 양쪽 다 트랙(302~346) 안에 대칭으로 들어옴

### 버그 2 — `/report`의 서비스 아이콘이 다른 화면과 다름
- 대시보드·등록 드롭다운·알림 화면은 전부 `ServiceIcon` 컴포넌트(브랜드 SVG, 없으면 이모지 폴백)를 쓰는데, `/report`의 "이번 주 사용량 체크인"과 "재구독 이력" 두 섹션만 `getServiceIcon()` 이모지를 직접 렌더링하고 있어서 넷플릭스 등 브랜드 아이콘이 있는 서비스도 리포트 화면에서만 이모지로 보이는 불일치가 있었음(2026-07-14 재구독 이력 추가 당시 "리포트 화면 안에서 아이콘 스타일을 섞지 않으려고" 의도적으로 이모지로 통일했던 결정이었으나, 이번에 다른 화면과의 일관성을 우선하기로 함)
- `src/app/report/page.tsx`의 두 `<span>{getServiceIcon(...)}</span>`를 `<ServiceIcon serviceName={...} size="md" />`로 교체, 더 이상 쓰이지 않는 `getServiceIcon` import를 `ServiceIcon` 컴포넌트 import로 변경
- Playwright로 넷플릭스 구독을 시딩해 대시보드와 리포트 양쪽에서 렌더링된 `<svg>`의 `outerHTML`(브랜드 path, `fill="#E50914"`)이 완전히 동일함을 확인

### 검증
- `npx tsc --noEmit`, `npm run build -- --webpack` 통과
- Playwright로 두 토글의 노브/트랙 좌표를 클릭 전후로 측정해 오버플로우 없음을 확인(375px 모바일 뷰포트)
- 넷플릭스 구독 시딩 후 대시보드·리포트 양쪽의 서비스 아이콘 SVG가 바이트 단위로 동일함을 확인
- 콘솔/페이지 에러 0건
